...
**Post-process `recovered_orphans/` to identify and rename files by type, and remove duplicate copies.**
...
4. Optionally removes byte-identical duplicate files (APFS hard links / COW clones)

### Usage

```bash
# Dry run — show what would be renamed/removed, but don't touch anything
python3 tools/identify_orphans.py <orphan_dir>

# Rename extensions and delete all-zero noise files
python3 tools/identify_orphans.py <orphan_dir> --rename

# Remove byte-identical duplicate files (keeps alphabetically-first name of each group)
python3 tools/identify_orphans.py <orphan_dir> --dedup

# Do both (dedup runs first, then rename applies only to surviving files)
python3 tools/identify_orphans.py <orphan_dir> --rename --dedup
```

The dry run always reports both rename candidates and duplicate groups so you can review before committing. `--rename` and `--dedup` are independent and composable.
