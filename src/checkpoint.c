...
void cp_save_scan(bool complete) {
    ...
    memset(counted, 0, g_max_inodes * sizeof(bool));
    for (int i = 0; i < g_drec_count; i++) {
...
bool cp_load_scan(void) {
    ...
    /* Accept CP_SCAN_VERSION 4 (no symlink_len field) and 5+ for backward
     * compatibility — a V4 checkpoint loaded with V5 code treats symlink_target
     * as NULL for all inodes, which is the same behaviour as before V5. */
    if (fread(&ver, 4, 1, f) != 1 ||
        (ver != 4 && ver != CP_SCAN_VERSION)) { fclose(f); return false; }
    ...
    /* Grow the hash table so its capacity stays ≥ 2× the inode count.
     * cp_load_scan grows g_inodes independently of get_or_create_inode_nolock,
     * so the hash table can be undersized after the loop above, causing silent
     * insertion failures once the table exceeds 100% fill. */
    if (g_inode_hash && g_max_inodes * 2 > g_inode_hash_capacity) {
        uint32_t new_cap = g_inode_hash_capacity;
        while (new_cap < g_max_inodes * 2) new_cap *= 2;
        free(g_inode_hash);
        g_inode_hash = calloc(new_cap, sizeof(inode_t *));
        if (g_inode_hash)
            g_inode_hash_capacity = new_cap;
    }
    ...
    for (uint32_t i = 0; i < ic; i++) {
        ...
        /* CP_SCAN_VERSION 5: symlink_target bytes follow decmpfs data.
...
void cp_save_extracted(uint64_t *ids, uint32_t count, const cp_extract_stats_t *stats) {
    ...
    uint32_t ver = CP_EXTRACT_VERSION;
...
uint32_t cp_load_extracted(bool *done_set, uint64_t *ids, uint32_t max_ids,
    ...
    if (fread(&ver,   4, 1, f) != 1 || ver != CP_EXTRACT_VERSION) { fclose(f); return 0; }
