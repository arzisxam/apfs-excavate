...
bool orphan_post_process(const char *orphan_dir, const char *output_dir, result_t *result) {
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        /* All-zero / all-0xFF check on decompressed or inline content.
         * The earlier raw-bytes noise guard only fires when buf[0] is 0x00
         * or 0xFF; compressed blobs with a valid header bypass it even when
         * the decompressed payload is pure noise.  Delete those here. */
        if (has_header && is_noise_content(content, content_len)) {
            free(raw);
            if (decomp_buf) free(decomp_buf);
            remove(src_path);
            zero_count++;
