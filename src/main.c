    ...
    if (!found) {
        ...
        if (count > 0) {
            ...
            for (int c = 0; c < count && !found; c++) {
                ...
                /* Sanity: XID (offset 8) must be non-zero on a live superblock. */
                if (get_u64(nb + 8) == 0) continue;
            ...
            if (!found && count > 0 && hit0_safe) {
                const uint8_t *nb0 = positions[0] - 32;
                {
                    ...
                    if (!found && cp_off >= 435ULL * det_bs) {
                        g_partition_offset = cp_off - 435ULL * det_bs;
                        g_block_size       = det_bs;
                        found = true;
                    }
                }
...
static uint64_t parse_size_arg(const char *s) {
    ...
    if (end == s || errno == ERANGE || val < 0.0) return 0;
    /* Clamp to avoid UB when casting +inf or out-of-range doubles to uint64_t. */
...
static void write_file_list(const char *output_dir) {
    ...
    if (g_filter_ext_count == 0) {
        for (int i = 0; i < g_inode_count; i++) {
            ...
            if (g_paths && g_paths[i]) continue; /* i is linear array index; g_paths is indexed the same way */
            if (g_min_file_size > 0 && ino->size < g_min_file_size) continue;
            if (ino->size > g_max_file_size) continue;
            orphan_count++;
            total_size += ino->size;
...
static void print_scan_summary_box(int total_files, uint64_t total_sz,
                                   bool scan_only) {
    char sf[32], ss_raw[32], ss_fmt[48];
    util_format_num((uint64_t)total_files, sf);
    snprintf(ss_fmt, sizeof(ss_fmt), "~%s", util_format_size(total_sz, ss_raw));
...
int main(int argc, char *argv[]) {
    ...
    int exit_code = 0;
    int fd = open(image_path, O_RDONLY | O_NOFOLLOW);
    if (fd < 0) {
