...
int recovery_extract_files(const char *files_dir, const char *orphan_dir,
    ...
    *compressed_count_out = 0;
    ...
    orphan_sig_t *orphan_sigs    = orphaned > 0
                                   ? calloc((size_t)orphaned, sizeof(orphan_sig_t))
                                   : NULL;
    ...
    for (int w = 0; w < work_count; w++) {
            ...
            for (int j = 0; j < ino->extent_count; j++) {
                ...
                for (uint64_t b = 0; b < ext_blocks; b++) {
                    ...
                    if (is_sparse) {
                        ...
                        if (!bio_read_block(phys_blk, raw_buf))
        ...
        if (is_orphan && orphan_sigs) {
            ...
            if (stat(full_path, &ost) == 0 && ost.st_size > 0) {
                size_t   sample = (size_t)(ost.st_size < 65536 ? ost.st_size : 65536);
                    ...
                    if (fh) {
                        ...
                        if (nr == sample) {
                            ...
                            if (sha_ctx &&
                                ...
                                EVP_DigestFinal_ex(sha_ctx, sig, &slen);

                            bool is_dup = false;
                            for (int k = 0; k < orphan_sig_cnt; k++) {
                            ...
                            if (!is_dup && orphan_sig_cnt < orphaned) {
